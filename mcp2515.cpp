...
MCP2515::ERROR MCP2515::reset(void)
{
    ...
    transaction.length = sizeof(INSTRUCTION_RESET) * BITS_IN_BYTE;
...
uint8_t MCP2515::getStatus(void)
{
    ...
    transaction.flags = SPI_TRANS_USE_RXDATA | SPI_TRANS_USE_TXDATA;
    transaction.tx_data[0] = INSTRUCTION_READ_STATUS;
...
MCP2515::ERROR MCP2515::setMode(const CANCTRL_REQOP_MODE mode)
{
    ...
    unsigned long endTime = pdTICKS_TO_MS(xTaskGetTickCount()) + 10;
    bool modeMatch = false;
    while (pdTICKS_TO_MS(xTaskGetTickCount()) < endTime)
    {
